...
int sosemanuk_test(void)
{
...

   {
       ...
       if ((err = sosemanuk_schedule(&kc, k, sizeof(k)))                             != CRYPT_OK) return err;
       if ((err = sosemanuk_init(&rc, &kc, n, sizeof(n)))                            != CRYPT_OK) return err;
       if ((err = sosemanuk_crypt(&rc, (unsigned char*)pt,       5,       out))      != CRYPT_OK) return err;
       if ((err = sosemanuk_crypt(&rc, (unsigned char*)pt +  5, 25,       out +  5)) != CRYPT_OK) return err;
       if ((err = sosemanuk_crypt(&rc, (unsigned char*)pt + 30, 10,       out + 30)) != CRYPT_OK) return err;
       if ((err = sosemanuk_crypt(&rc, (unsigned char*)pt + 40, len - 40, out + 40)) != CRYPT_OK) return err;
       ...
       if ((err = sosemanuk_schedule(&kc, k, sizeof(k)))              != CRYPT_OK) return err;
       if ((err = sosemanuk_init(&rc, &kc, n, sizeof(n)))             != CRYPT_OK) return err;
       if ((err = sosemanuk_crypt(&rc, (unsigned char*)pt, len, out)) != CRYPT_OK) return err;
   ...
   }
   {
       ...
       if ((err = sosemanuk_schedule(&kc, k3, sizeof(k3)))   != CRYPT_OK)     return err;
       if ((err = sosemanuk_init(&rc, &kc, n3, sizeof(n3)))  != CRYPT_OK)     return err;
       if ((err = sosemanuk_keystream(&rc, out, 64))         != CRYPT_OK)     return err;
       if ((err = sosemanuk_done(&kc, &rc))                  != CRYPT_OK)     return err;
